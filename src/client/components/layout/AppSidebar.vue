...
interface Appointment {
  ...
  location?: string;
  calendarId: string;
  recursionRule?: number;
  recursionEndDate?: string;
}

const emit = defineEmits<{
  (e: 'close'): void;
  (e: 'selectAppointment', appointment: any): void;
  (e: 'toggleappointmentsdisplay'): void;
  (e: 'togglecalendarsdisplay'): void;
  (e: 'toggletagsdisplay'): void;
  (e: 'CalendarForm'): void;
  (e: 'TagForm'): void;
  (e: 'editCalendar', id: string, name: string, description: string, color: string): void;
  (e: 'deleteCalendar', id: string): void;
  (e: 'editTag', tag: Tag): void;
  (e: 'deleteTag', id: string): void;
  (e: 'exportCalendar', calendar: Calendar, appointments: Appointment[]): void;
  (e: 'ImportCalendar'): void;
  (e: 'calendarToggled', id: string, visible: boolean): void;
  (e: 'removeSharedCalendar', id: string): void;
  (e: 'calendarsUpdated'): void;
}>();

const props = defineProps<{
  isOpen: boolean;
  appointmentsdisplayed: boolean;
  calendarsdisplayed: boolean;
...
const getCalendarById = (id: string | null): Calendar | null => {
  if (!id) return null;
  return props.calendars.find((c) => c.id === id) || null;
};

const getSharedCalendarById = (id: string | null): Calendar | null => {
  if (!id) return null;
  return props.sharedCalendars.find((c) => c.id === id) || null;
};

const toggleDropdown = (calendarId: string, event: Event, isShared: boolean = false) => {
...
const handleExport = (calendar: Calendar | null) => {
  if (!calendar) return;
...
const handleRemoveSharedCalendar = async (calendarId: string) => {
  try {
    ...
    const calendar = props.sharedCalendars.find((c) => c.id === calendarId);
    if (calendar) {
...
const isSharedAppointment = (appointment: any): boolean => {
  if (!appointment?.calendarId) return false;
  return props.sharedCalendars.some((c) => c.id === appointment.calendarId);
};

const getAppointmentCalendarColor = (appointment: any): string => {
  if (!appointment?.calendarId) return '#6366f1';

  const personalCalendar = props.calendars.find((c) => c.id === appointment.calendarId);
  if (personalCalendar) return personalCalendar.color || '#6366f1';

  const sharedCalendar = props.sharedCalendars.find((c) => c.id === appointment.calendarId);
  if (sharedCalendar) return sharedCalendar.color || '#6366f1';
...
const selectTab = (view: typeof tabOrder[number], emitName: 'toggleappointmentsdisplay' | 'togglecalendarsdisplay' | 'toggletagsdisplay') => {
  internalView.value = view;
  emit(emitName);
};

const dispatchTagFormEvent = (payload: Tag | null = null) => {
...
const getTagById = (id: string): Tag | undefined => props.tags.find((t) => t.id === id);
const getTagColor = (id: string): string => getTagById(id)?.color || '#ccc';
const getTagName = (id: string): string => getTagById(id)?.name || 'Inconnu';

const shareModalCalendar = ref<Calendar | null>(null);
const isShareModalOpen = computed(() => shareModalCalendar.value !== null);

const handleShare = (calendar: Calendar | null) => {
  if (!calendar) return;
...
const handleCalendarToggle = (calendarId: string, event: Event) => {
  const target = event.target as HTMLInputElement;
  emit('calendarToggled', calendarId, target.checked);
};
</script>
          ...
          <div class="flex-1 overflow-y-auto p-4">
            ...
            <div v-else class="space-y-3">
              <div
                  ...
                  :class="[
                        'p-3 border rounded-xl hover:shadow-md transition-all cursor-pointer group relative',
                        isSharedAppointment(appointment)
              ...
              >
                <!-- Badge partagé -->
                <div v-if="isSharedAppointment(appointment)" class="absolute -top-2 -right-2">
                ...
                <div class="flex items-start gap-3">
                  <div
                      ...
                      :style="{ backgroundColor: getAppointmentCalendarColor(appointment) }"
                  ></div>
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3 flex-1 min-w-0">
                    <input
                        ...
                        @change="handleCalendarToggle(calendar.id, $event)"
                        class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                    />
                    <div
                        class="w-4 h-4 rounded-full flex-shrink-0 ring-2 ring-offset-1"
                        :style="{ backgroundColor: calendar.color, '--tw-ring-color': calendar.color + '40' } as any"
                    ></div>
              ...
              >
                <div class="flex items-center justify-between gap-2">
                  <div class="flex items-center gap-3 flex-1 min-w-0">
                    <div
                        class="w-4 h-4 rounded-full flex-shrink-0 ring-2 ring-offset-1"
                        :style="{ backgroundColor: tag.color, '--tw-ring-color': tag.color + '40' } as any"
                    ></div>
