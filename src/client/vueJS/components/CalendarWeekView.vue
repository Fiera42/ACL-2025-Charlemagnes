      ...
      <div class="flex flex-col md:flex-row max-md:gap-3 items-center justify-between mb-5">
        ...
        <div class="flex items-center gap-px rounded-lg bg-gray-100 p-1">
          <button
              ...
              :key="view"
              class="rounded-lg py-2.5 px-5 text-sm font-medium transition-all duration-300"
              :class="{
              'bg-white text-indigo-600': currentView === view,
              'text-gray-500 hover:bg-white hover:text-indigo-600': currentView !== view
            }"
      ...
      <div class="relative">
        ...
        <div class="grid grid-cols-7 border-t border-gray-200 sticky top-0 left-0 w-full">
          <div
              ...
              :class="{
              'text-indigo-600': isToday(day.date),
        ...
        <div class="hidden grid-cols-7 sm:grid w-full overflow-x-auto">
          <template v-for="(hour, hIndex) in hours" :key="hIndex">
            <div
            ...
            >
              <div
                  v-for="event in getEventsFor(day.date, hour)"
                  :key="event.title"
                  class="rounded p-1.5 border-l-2"
                  :class="event.colorClass"
              >
                <p class="text-xs font-normal text-gray-900 mb-px">{{ event.title }}</p>
                <p class="text-xs font-semibold" :class="event.textColor">
                  {{ event.time }}
                </p>
              </div>
            </div>
...
const weekDays = computed(() => {
  ...
  return Array.from({ length: 7 }).map((_, i) => {
    ...
    return {
      date,
      label: date.toLocaleDateString('fr', { month: 'short', day: 'numeric' })
    }
  })
})
...
const formattedDate = computed(() => {
  const today = new Date()
  return today.toLocaleDateString('fr', { day: 'numeric',month: 'long', year: 'numeric' })
})
...
function getEventsFor(date, hour) {
  return props.events.filter(e => e.date === date.toDateString() && e.hour === hour)
}
</script>