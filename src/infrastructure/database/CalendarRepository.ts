...
export class CalendarRepository implements ICalendarDB {
  ...
  async createCalendar(calendar: Calendar): Promise<Calendar> {
    ...
    await pool.execute<ResultSetHeader>(
      'INSERT INTO calendars (id, name, description, color, owner_id, created_at) VALUES (?, ?, ?, ?, ?, ?)',
      [id, calendar.name, calendar.description, calendar.color, calendar.ownerId, calendar.createdAt]
    );

    return new Calendar(id, calendar.name, calendar.description, calendar.color, calendar.ownerId, calendar.createdAt);
  }
  ...
  async findCalendarById(id: string): Promise<Calendar | null> {
    ...
    const row = rows[0];
    return new Calendar(row.id, row.name, row.description, row.color, row.owner_id, new Date(row.created_at));
  }
  ...
  async findCalendarsByOwnerId(ownerId: string): Promise<Calendar[]> {
    ...
    return rows.map(row => new Calendar(row.id, row.name, row.description, row.color, row.owner_id, new Date(row.created_at)));
  }
  ...
  // Appointments (inchangés)
  async createAppointment(appointment: Appointment): Promise<Appointment> {
    const id = uuidv4();
    await pool.execute<ResultSetHeader>(
      'INSERT INTO appointments (id, calendar_id, title, description, start_date, end_date, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [id, appointment.calendarId, appointment.title, appointment.description, appointment.startDate, appointment.endDate, appointment.createdAt]
    );

    return new Appointment(id, appointment.calendarId, appointment.title, appointment.description, appointment.startDate, appointment.endDate, appointment.createdAt);
  }
  ...
  async findAppointmentById(id: string): Promise<Appointment | null> {
    ...
    const row = rows[0];
    return new Appointment(row.id, row.calendar_id, row.title, row.description, new Date(row.start_date), new Date(row.end_date), new Date(row.created_at));
  }
  ...
  async findAppointmentsByCalendarId(calendarId: string): Promise<Appointment[]> {
    ...
    return rows.map(row => new Appointment(row.id, row.calendar_id, row.title, row.description, new Date(row.start_date), new Date(row.end_date), new Date(row.created_at)));
  }
